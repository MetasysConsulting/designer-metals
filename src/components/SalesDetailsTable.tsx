...
export default function SalesDetailsTable({ filters }: { filters: any }) {
  ...
  const fetchSalesDetails = async () => {
    try {
      ...
      const allData = await clientCSVDataSource.getFilteredData(filters);
      
      // Sort by date descending and limit to 100 for performance
      const sortedData = allData
        .filter(record => record.INV_DATE && record.TOTAL)
        .sort((a, b) => new Date(b.INV_DATE).getTime() - new Date(a.INV_DATE).getTime())
        .slice(0, 100);
      
      console.log('Sales details data from CSV:', sortedData.length, 'records');

      // Transform data
      const transformedData: SalesDetail[] = sortedData.map(record => ({
        ...
        subtotal: parseFloat(record.SUBTOTAL || '0') || 0,
        taxAmount: parseFloat(record.TAX || '0') || 0,
        total: parseFloat(record.TOTAL || '0') || 0,
        totalPaid: parseFloat(record.TOTAL_PAID || '0') || 0,
