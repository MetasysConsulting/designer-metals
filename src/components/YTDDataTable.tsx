...
export default function YTDDataTable({ filters }: YTDDataTableProps) {
  ...
  const fetchYTDTableData = async () => {
    try {
      ...
      if (error) {
        console.error('Error fetching YTD table data:', error)
        throw new Error(`Failed to fetch YTD table data: ${error.message}`)
      }

      console.log('YTD table data fetched successfully:', records?.length, 'records')
