...
export async function exportToPDF(title: string = 'Designer Metals Dashboard') {
  try {
    ...
    const canvas = await html2canvas(dashboardElement, {
      scale: 1.5, // Slightly lower scale for better performance with large content
      useCORS: true,
      allowTaint: true,
    ...
    // Save the PDF
    const fileName = `designer-metals-dashboard-${new Date().toISOString().split('T')[0]}.pdf`
    pdf.save(fileName)
